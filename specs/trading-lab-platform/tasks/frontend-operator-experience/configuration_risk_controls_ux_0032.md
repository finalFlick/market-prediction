...
Define how configuration, risk limits, kill switch, and approval state are surfaced and eventually edited through the dashboard.

## Why

The UI becomes the config source of truth over time, but risk-sensitive settings need staged writes and auditability.

## What

A configuration UX capability covering read-only MVP-0 display, v1 write flows, risk warnings, import/export, and approval hooks.

## User / System Story

As a platform operator or implementation agent,
I want configuration and risk controls ux,
so that the trading-lab platform can satisfy the related requirements with evidence.

## Source Traceability

- requirements.md: Req 24, 25, 33, 34
- design.md: UI source of truth; RiskLimitsForm; KillSwitchControls; approval queue
...
UI source of truth; RiskLimitsForm; KillSwitchControls; approval queue. This feature states the expected capability and review evidence without prescribing code-level implementation details beyond the design and project rules.
...
- form validation tests; read-only/write phase tests; audit event checks
...
- backend config API; approval policy; audit log

## Risks / Edge Cases

- Editing risk controls too early can undermine safety.
