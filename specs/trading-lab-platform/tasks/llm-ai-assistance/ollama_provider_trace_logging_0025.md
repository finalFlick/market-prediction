...
Define the MVP-0 local LLM provider contract and trace logging for summaries and research assistance.

## Why

LLMs are useful, but MVP-0 must remain local, auditable, and isolated.

## What

A provider capability for Ollama-only MVP-0, prompt hash logging, model/config metadata, fallback behavior, and nondeterministic summary labeling.

## User / System Story

As a platform operator or implementation agent,
I want ollama provider and llm trace logging,
so that the trading-lab platform can satisfy the related requirements with evidence.

## Source Traceability

- requirements.md: Req 22, 23, 39, 41
- design.md: Component 12; Prompt / Model Traceability; Security Considerations
...
Component 12; Prompt / Model Traceability; Security Considerations. This feature states the expected capability and review evidence without prescribing code-level implementation details beyond the design and project rules.
...
- LLM isolation tests; trace log assertions; provider fallback tests
...
- monitoring.logger; OLLAMA env config; research.llm client

## Risks / Edge Cases

- LLM outputs can leak into execution decisions.
