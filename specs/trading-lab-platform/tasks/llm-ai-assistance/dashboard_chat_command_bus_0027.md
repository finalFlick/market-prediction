...
Define v1 dashboard chat as an operator assistance and command-request interface with safety boundaries.

## Why

Chat can help beginner operators, but must not become a direct order or promotion surface.

## What

A chat capability for read queries, safe command proposals, session history, and blocked high-risk requests.

## User / System Story

As a platform operator or implementation agent,
I want dashboard chat command bus,
so that the trading-lab platform can satisfy the related requirements with evidence.

## Source Traceability

- requirements.md: Req 10, 21, 23, 34
- design.md: Component 13; API command pattern; chat isolation
...
Component 13; API command pattern; chat isolation. This feature states the expected capability and review evidence without prescribing code-level implementation details beyond the design and project rules.
...
- chat authorization tests; forbidden action tests; UX review
...
- backend API; LLM provider; approval queue; frontend

## Risks / Edge Cases

- Chat responses can sound like actionable trade advice.
