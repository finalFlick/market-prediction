...
Define how mutating API actions become commands consumed by the engine instead of direct state changes.

## Why

Commands that affect runs, approvals, or risk must be auditable and ordered.

## What

A command capability for request validation, idempotency, command records, status reporting, and retry/failure semantics.

## User / System Story

As a platform operator or implementation agent,
I want asynchronous mutating command pattern,
so that the trading-lab platform can satisfy the related requirements with evidence.

## Source Traceability

- requirements.md: Req 10, 17, 35, 41
- design.md: Asynchronous Command Pattern; Redis Streams schemas; Error Handling
...
Asynchronous Command Pattern; Redis Streams schemas; Error Handling. This feature states the expected capability and review evidence without prescribing code-level implementation details beyond the design and project rules.
...
- idempotency tests; command lifecycle tests; failure/retry tests
...
- backend API; run orchestrator; audit log; Redis streams

## Risks / Edge Cases

- Duplicate submissions can create duplicate commands.
