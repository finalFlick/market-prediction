...
Define SSE/WebSocket streams for run progress, alerts, metrics, and dashboard freshness.

## Why

The operator needs timely feedback during long-running and safety-sensitive workflows.

## What

A realtime capability for event schemas, auth, stream latency, reconnect behavior, and stale-client handling.

## User / System Story

As a platform operator or implementation agent,
I want realtime run event streaming,
so that the trading-lab platform can satisfy the related requirements with evidence.

## Source Traceability

- requirements.md: Req 35, 41, 32
- design.md: Component 20; Event Schemas; Frontend Realtime Update Strategy; SLOs
...
Component 20; Event Schemas; Frontend Realtime Update Strategy; SLOs. This feature states the expected capability and review evidence without prescribing code-level implementation details beyond the design and project rules.
...
- stream contract tests; latency smoke tests; reconnect/degraded tests
...
- run events; backend auth; frontend dashboard

## Risks / Edge Cases

- Streams can leak data if auth is weak.
