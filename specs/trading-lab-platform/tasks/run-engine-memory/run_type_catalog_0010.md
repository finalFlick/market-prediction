...
Define supported run types, phase, trigger policy, outputs, and safety boundaries.

## Why

Run types carry different evidence and risk semantics and need explicit contracts.

## What

A catalog for BacktestRun, PaperRun, StrategyTestRun, MarketScoutRun, and PortfolioReviewRun behavior by phase.

## User / System Story

As a platform operator or implementation agent,
I want mvp-0 and v1 run type catalog,
so that the trading-lab platform can satisfy the related requirements with evidence.

## Source Traceability

- requirements.md: Req 10, 11, 12, 25
- design.md: MVP-0 Deliverables; Component 3; Build Plan
...
MVP-0 Deliverables; Component 3; Build Plan. This feature states the expected capability and review evidence without prescribing code-level implementation details beyond the design and project rules.
...
- run type contract tests; trigger authorization tests; artifact presence checks
...
- run lifecycle; risk/execution safety; backend route inventory

## Risks / Edge Cases

- AI-triggered runs can be over-authorized.
